...
static esp_err_t btn_post_handler(httpd_req_t *req) {
    ...
    cJSON *jreset = cJSON_GetObjectItem(root, "is_reset");
    ...
    if (cJSON_IsNumber(jid) && cJSON_IsString(jtarget)) {
        ...
        if (cJSON_IsString(jname)) {
            strlcpy(cfg.name, jname->valuestring, sizeof(cfg.name));
        }
        cfg.is_reset = cJSON_IsBool(jreset) ? cJSON_IsTrue(jreset) : false;
...
static esp_err_t btn_get_handler(httpd_req_t *req) {
    ...
    if (app_nvs_get_button_config(btn_id, &cfg) == ESP_OK) {
        ...
        cJSON_AddBoolToObject(root, "is_reset", cfg.is_reset);
    }
