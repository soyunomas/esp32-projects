...
<div class="tabs" id="btn-tabs">
<div class="tab act" onclick="swTab(1,this)">B1</div>
<div class="tab" onclick="swTab(2,this)">B2</div>
<div class="tab" onclick="swTab(3,this)">B3</div>
<div class="tab" onclick="swTab(4,this)">B4</div>
<div class="tab" onclick="swTab(5,this)">B5</div>
</div>
<label>Nombre</label>
<input id="bname" placeholder="Ej: Luz salón" maxlength="31">
<div class="chk-row">
<input type="checkbox" id="bis_reset" onchange="chkReset()">
<label for="bis_reset">Usar para Factory Reset (mantener pulsados)</label>
</div>
...
if(r.ok){$('login').classList.add('hid');$('app').classList.remove('hid');$('auser').value=u;curP=p;loadBtn(1);loadSys();loadMqtt();updTabNames();}
...
function chkReset(){
var c=$('bis_reset').checked;rstMap[cBtn]=c;
var cnt=0;for(var k in rstMap){if(rstMap[k])cnt++;}
if(cnt>2){$('bis_reset').checked=false;rstMap[cBtn]=false;msg('bmsg',0,'Máximo 2 botones de reset');}
}
function updTabNames(){
var tabs=$('btn-tabs').children;
for(var i=0;i<tabs.length;i++){var id=i+1;
af('/api/btn?id='+id).then((function(idx,tab){return function(r){return r.json().then(function(d){
rstMap[idx]=!!d.is_reset;tab.textContent=d.name||('B'+idx);
});};})(id,tabs[i])).catch(function(){});
}}
function gBD(){return{id:cBtn,atype:parseInt($('batype').value),target:$('btarget').value,method:parseInt($('bmethod').value),payload:$('bpayload').value||'',timeout:parseInt($('btimeout').value||'5')*1000,cooldown:parseFloat($('bcooldown').value||'2')*1000,nocache:$('bnocache').checked,name:$('bname').value,is_reset:$('bis_reset').checked};}
function saveBtn(){
...
}).then(function(r){return r.json()}).then(function(d){msg('bmsg',d.ok,d.ok?'Boton '+cBtn+' guardado':'Error');if(d.ok)updTabNames();
...
$('bname').value=d.name||'';$('bis_reset').checked=!!d.is_reset;rstMap[n]=!!d.is_reset;
