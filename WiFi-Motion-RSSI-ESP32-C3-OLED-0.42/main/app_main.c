...
void app_main(void)
{
    ...
    const esp_err_t oled_error = oled_status_init();
    if (oled_error == ESP_OK) {
        oled_status_show_boot();
    } else {
        ESP_LOGW(TAG,
                 "OLED unavailable; continuing headless: %s",
                 esp_err_to_name(oled_error));
    }
    init_motion_led();
    init_event_marker();
    ...
    char display_ip[16] = "192.168.4.1";
    char display_network[19] = "";
    if (!provisioning_mode) {
        esp_netif_ip_info_t ip_info;
        if (esp_netif_get_ip_info(wifi_sta_netif, &ip_info) == ESP_OK) {
            esp_ip4addr_ntoa(&ip_info.ip, display_ip, sizeof(display_ip));
        } else {
            strlcpy(display_ip, "NO IP", sizeof(display_ip));
        }
    } else {
        uint8_t station_mac[6];
        if (esp_wifi_get_mac(WIFI_IF_STA, station_mac) == ESP_OK) {
            snprintf(display_network,
                     sizeof(display_network),
                     "WIFI-MOTION-%02X%02X%02X",
                     station_mac[3],
                     station_mac[4],
                     station_mac[5]);
    ...
    oled_status_show_network(display_ip,
                             provisioning_mode,
                             display_network);
    ...
    int64_t last_oled_update_ms = -1000;
    ...
    for (;;) {
        ...
        if (error == ESP_OK) {
            ...
            const bool selected_motion_active = selected_detector_active(
                ...
                &latest_result,
                &latest_csi_result);
            if (timestamp_ms - last_oled_update_ms >= 500 ||
                selected_motion_active != selected_motion_was_active) {
                const bool selected_calibrated =
                    runtime_config.detection_source == APP_DETECTION_SOURCE_RSSI
                        ? latest_result.calibrated
                        : runtime_config.detection_source ==
                                  APP_DETECTION_SOURCE_CSI
                            ? latest_csi_result.calibrated
                            : latest_result.calibrated &&
                                  latest_csi_result.calibrated;
                oled_status_show_sample(display_ip,
                                        true,
                                        ap_info.rssi,
                                        selected_calibrated,
                                        selected_motion_active,
                                        latest_result.score,
                                        latest_csi_result.score);
                last_oled_update_ms = timestamp_ms;
            }
            ...
            if (!provisioning_mode &&
                timestamp_ms - last_oled_update_ms >= 500) {
                oled_status_show_sample(display_ip,
                                        false,
                                        0,
                                        false,
