# WiFi Motion RSSI — ESP32-C3 OLED 0.42 motion sensing
...
This variant targets the 01Space ESP32-C3 0.42 OLED board and compatible clones.
It includes RSSI and CSI detection, adaptive calibration, an onboard status
screen, a responsive English/Spanish web interface, live charts, Wi-Fi recovery,
administrator authentication, Telegram notifications, and telemetry.
...
- [Supported board and OLED](#supported-board-and-oled)
...
## Supported board and OLED

The connected board was identified as the **01Space ESP32-C3 0.42 OLED** family:

- ESP32-C3 revision 0.4 with 4 MB embedded flash;
- native Espressif USB Serial/JTAG (`303a:1001`);
- 0.42-inch, 72×40 OLED with an SSD1306-compatible controller at I2C address
  `0x3C`;
- OLED `SDA=GPIO5`, `SCL=GPIO6`;
- BOOT/experiment marker on GPIO9;
- WS2812 RGB LED on GPIO2. This firmware leaves it unused and uses the OLED for
  local status.

The OLED uses a centered, high-contrast 5×7 font enlarged to 2× pixel scale.
The two large lines rotate between motion state/RSSI, the complete IP address
(including a separate `IP END` page when it exceeds 12 characters), and
RSSI/CSI scores. In setup mode they prioritize `SETUP` and the final IP octet (`1` for
`192.168.4.1`). A missing display is reported over serial and does not stop motion
detection or the web portal.

Hardware references: [Zephyr board documentation](https://pigweed.googlesource.com/third_party/github/zephyrproject-rtos/zephyr/+/refs/tags/v4.2.1/boards/01space/esp32c3_042_oled/doc/index.rst)
and [U8g2's SSD1306 72×40 driver](https://github.com/olikraus/u8g2/blob/master/csrc/u8x8_d_ssd1306_72x40.c).
...
The [`firmware/`](firmware/) directory contains binaries for the 01Space
ESP32-C3 0.42 OLED board with 4 MB flash. No ESP-IDF checkout is required; only
Python and Espressif's `esptool` are needed.
...
py -m esptool --chip esp32c3 --port COM5 --baud 460800 `
  ...
  0x0 firmware/wifi-motion-rssi-c3-oled-042-complete.bin
```
...
- 01Space ESP32-C3 0.42 OLED or compatible clone with 4 MB flash;
- USB data cable;
...
AP password, button GPIO, CSI, controlled traffic, and telemetry format. The
OLED pins are fixed to the board wiring. Runtime settings can later be changed
from the portal.
