# WiFi Motion RSSI — sensor con ESP32-C3 OLED de 0,42″
...
Esta variante está destinada a la placa 01Space ESP32-C3 0.42 OLED y clones
compatibles. Incluye detector RSSI, captura CSI, calibración adaptativa, estado
en la pantalla integrada, gráfica web, selección de red Wi-Fi, acceso de
administrador, notificaciones por Telegram y telemetría.
...
- [Placa y pantalla compatibles](#placa-y-pantalla-compatibles)
...
## Placa y pantalla compatibles

El dispositivo conectado se identificó como parte de la familia **01Space
ESP32-C3 0.42 OLED**:

- ESP32-C3 revisión 0.4 con 4 MB de flash integrada;
- USB Serial/JTAG nativo de Espressif (`303a:1001`);
- OLED de 0,42 pulgadas y 72×40, controlador compatible con SSD1306 y dirección
  I²C `0x3C`;
- pantalla conectada a `SDA=GPIO5` y `SCL=GPIO6`;
- botón BOOT/hito experimental en GPIO9;
- WS2812 RGB en GPIO2. Este firmware no lo utiliza: el estado local se muestra
  en la OLED.

La pantalla emplea una fuente 5×7 centrada, de alto contraste y ampliada al
doble de píxeles. Sus dos líneas grandes alternan estado/RSSI, la dirección IP
completa —con una página `IP END` para los dígitos posteriores al carácter
12— y scores RSSI/CSI. En configuración prioriza `SETUP` y el último octeto de la IP
(`1` para `192.168.4.1`). Si no se detecta la OLED, el firmware lo registra por
serie y continúa funcionando con
el portal web.

Referencias de hardware: [documentación de la placa en Zephyr](https://pigweed.googlesource.com/third_party/github/zephyrproject-rtos/zephyr/+/refs/tags/v4.2.1/boards/01space/esp32c3_042_oled/doc/index.rst)
y [controlador SSD1306 72×40 de U8g2](https://github.com/olikraus/u8g2/blob/master/csrc/u8x8_d_ssd1306_72x40.c).
...
La carpeta [`firmware/`](firmware/) contiene una versión ya compilada para la
01Space ESP32-C3 0.42 OLED con 4 MB de flash. La forma más sencilla desde Linux
es:
...
- 01Space ESP32-C3 0.42 OLED o clon compatible con 4 MB de flash;
- cable USB de datos;
