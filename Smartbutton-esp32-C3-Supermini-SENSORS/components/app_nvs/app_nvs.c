...
esp_err_t app_nvs_save_button(int btn_id, button_config_t *config) {
    ...
    nvs_set_i32(my_handle, "gpio", config->gpio_num);
    nvs_set_i32(my_handle, "inmode", config->input_mode);
    nvs_set_i32(my_handle, "stab_ms", config->stabilization_ms);
    nvs_set_u8(my_handle, "is_reset", config->is_reset_btn ? 1 : 0);
    nvs_set_u8(my_handle, "enabled", config->enabled ? 1 : 0);
    nvs_set_i32(my_handle, "atype", config->action_type);
    nvs_set_str(my_handle, "url", config->target);
...
esp_err_t app_nvs_get_button_config(int btn_id, button_config_t *config) {
    ...
    tmp32 = -1;
    if (nvs_get_i32(my_handle, "gpio", &tmp32) == ESP_OK) config->gpio_num = (int)tmp32;
    else config->gpio_num = -1;

    tmp32 = INPUT_MODE_BUTTON;
    if (nvs_get_i32(my_handle, "inmode", &tmp32) == ESP_OK) config->input_mode = (int)tmp32;
    else config->input_mode = INPUT_MODE_BUTTON;

    tmp32 = 0;
    if (nvs_get_i32(my_handle, "stab_ms", &tmp32) == ESP_OK) config->stabilization_ms = (int)tmp32;
    else config->stabilization_ms = 0;

    uint8_t u8tmp = 0;
    if (nvs_get_u8(my_handle, "is_reset", &u8tmp) == ESP_OK) config->is_reset_btn = (u8tmp == 1);
    else config->is_reset_btn = false;

    u8tmp = 1;
    if (nvs_get_u8(my_handle, "enabled", &u8tmp) == ESP_OK) config->enabled = (u8tmp == 1);
