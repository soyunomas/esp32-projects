...
static esp_err_t btn_post_handler(httpd_req_t *req) {
    ...
    cJSON *jgpio = cJSON_GetObjectItem(root, "gpio");
    cJSON *jinmode = cJSON_GetObjectItem(root, "inmode");
    cJSON *jstab = cJSON_GetObjectItem(root, "stab");
    cJSON *jisreset = cJSON_GetObjectItem(root, "is_reset");
    ...
    if (cJSON_IsNumber(jid)) {
        ...
        cfg.gpio_num = cJSON_IsNumber(jgpio) ? jgpio->valueint : -1;
        cfg.input_mode = cJSON_IsNumber(jinmode) ? jinmode->valueint : 0;
        cfg.stabilization_ms = cJSON_IsNumber(jstab) ? jstab->valueint : 0;
        if (cfg.stabilization_ms < 0) cfg.stabilization_ms = 0;
        if (cfg.stabilization_ms > 120000) cfg.stabilization_ms = 120000;
        cfg.is_reset_btn = cJSON_IsBool(jisreset) ? cJSON_IsTrue(jisreset) : false;
        cfg.enabled = cJSON_IsBool(jenabled) ? cJSON_IsTrue(jenabled) : true;
        ...
        if (cfg.cooldown_ms > 600000) cfg.cooldown_ms = 600000;
...
static esp_err_t btn_get_handler(httpd_req_t *req) {
    ...
    if (app_nvs_get_button_config(btn_id, &cfg) == ESP_OK) {
        cJSON_AddNumberToObject(root, "gpio", cfg.gpio_num);
        cJSON_AddNumberToObject(root, "inmode", cfg.input_mode);
        cJSON_AddNumberToObject(root, "stab", cfg.stabilization_ms);
        cJSON_AddBoolToObject(root, "is_reset", cfg.is_reset_btn);
