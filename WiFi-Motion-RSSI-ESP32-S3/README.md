# WiFi Motion RSSI — ESP32-S3 motion sensing over Wi-Fi

**English** | [Español](README-ES.md)

WiFi Motion RSSI detects motion-compatible changes in a Wi-Fi radio link. An
ESP32-S3 observes how the channel between the access point and the board changes;
...
- [Verified ESP32-S3 configuration](#verified-esp32-s3-configuration)
...
## Verified ESP32-S3 configuration

The ESP32-S3 variant has been built, flashed, and exercised on this hardware:
...
| Embedded PSRAM | 8 MB |
| USB serial bridge | QinHeng USB Single Serial (`1a86:55d3`) |
| Linux serial port | `/dev/ttyACM0` |
| Console and telemetry | UART0 at 115200 baud |
| Wi-Fi | 2.4 GHz WPA2-PSK |

The firmware image intentionally targets a minimum 4 MB flash layout, so it can
run on S3 boards with 4 MB or more. Extra flash and PSRAM are not currently
required.

The original C3 firmware wrote telemetry directly to USB Serial/JTAG. This
variant uses standard output over UART0 so boards with a USB-to-UART bridge can
flash and monitor through the same connector. GPIO0 is the default active-low
BOOT marker/recovery input. The status LED is disabled by default because S3
boards use different GPIOs and may contain either a conventional LED or an
addressable RGB LED.
...
```text
Access point  ~~~~~ direct path ~~~~~>  ESP32-S3
       \                              /
...
A fresh installation contains no home Wi-Fi credentials, so first boot starts:
...
After joining the router, its address is assigned by DHCP. Read the `sta ip:`
line from the serial monitor or check the router's client list. For example, the
hardware validation session received `192.168.24.122`; this is not a fixed
address and may change.
...
- ESP32-S3 development board with at least 4 MB flash;
- USB data cable connected to the board's USB-to-UART port;
- USB-UART bridge wired to UART0 (ESP32-S3 TX GPIO43 and RX GPIO44 on the
  verified board);
- Linux or macOS shell environment;
- Git, CMake, a C compiler, and Python 3 for host tests;
- Internet access during the first dependency installation.

This source variant is tested with ESP-IDF commit
`12f36a021f511cd4de41d3fffff146c5336ac1e7`, described locally as
`v6.1-dev-2938-g12f36a021f`.

The status LED is disabled by default because its GPIO and electrical type vary
between ESP32-S3 boards. GPIO0 is used for the active-low BOOT marker/recovery
button. It may be changed or disabled in `idf.py menuconfig`.
...
the ESP32-S3 toolchain. Override `IDF_ROOT` or `IDF_REF` only when intentionally
...
```text
wifi_motion: Connected to configured WiFi AP
esp_netif_handlers: sta ip: 192.168.x.x
csi_capture: CSI capture enabled for connected AP
csi_traffic: Controlled CSI traffic started
```
...
The current ESP32-S3 port passes all 12 host suites. Hardware validation also
confirmed continuous ICMP responses and repeated HTTP `200` responses after CSI
started.
...
### The board reboots when CSI starts

Look for `A stack overflow in task csi_processing` in the serial output. The S3
port allocates 6144 bytes to this task because the feature extractor, maximum CSI
frame, temporary float array, and floating-point call chain do not fit safely in
the original C3 allocation of 3072 bytes. Rebuild and flash the current source
instead of an earlier S3 binary.
...
On ESP32-S3, the CSI processing task uses a 6144-byte stack. This was verified
after the original 3072-byte C3 allocation caused an immediate stack overflow
when controlled CSI traffic began.
...
- ESP32-S3 port: UART0 console, Wi-Fi association, DHCP, CSI processing, ICMP
  reachability, and HTTP portal verified on hardware.
