# WiFi Motion RSSI — sensor de movimiento con ESP32-S3

[English](README.md) | **Español**

Detector de movimiento que utiliza cambios en una señal Wi-Fi. El
ESP32-S3 no necesita cámara, micrófono ni un sensor PIR: observa cómo varía el
...
```text
Router  ~~~~~~~ camino directo ~~~~~~~>  ESP32-S3
   \                                  /
...
- funciona con un ESP32-S3 y un router normal;
...
- placa de desarrollo ESP32-S3 con al menos 4 MB de flash;
- cable de datos conectado al puerto USB-UART de la placa;
- Linux, CMake, compilador C y Python 3 para las pruebas host;
- ESP-IDF en el commit `12f36a021f511cd4de41d3fffff146c5336ac1e7`
  (`v6.1-dev-2938-g12f36a021f`), que es el entorno usado para probar esta
  variante;
...
El LED de estado queda desactivado por defecto porque su GPIO y su tipo eléctrico
varían entre placas ESP32-S3. GPIO0 se usa como botón BOOT activo en bajo para
marcado y recuperación; ambos valores pueden cambiarse con `idf.py menuconfig`.
...
No pulses BOOT durante el funcionamiento normal. GPIO0 es un pin de arranque:
