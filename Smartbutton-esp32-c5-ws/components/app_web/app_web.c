...
#include "app_http.h"
#include "app_mqtt.h"
#include "app_ws.h" // Include nuevo
#include "esp_log.h"
#include "cJSON.h"
...
static esp_err_t test_post_handler(httpd_req_t *req) {
    ...
    } else if (cfg.action_type == 2) {
        // WebSocket Test
        status = app_ws_test_sync(&cfg);
    } else {
        // HTTP Test
    ...
    if (cfg.action_type == 1) {
        ...
        if (status == -1) cJSON_AddStringToObject(resp, "msg", "Error conexión WebSocket");
        else if (status == 504) cJSON_AddStringToObject(resp, "msg", "Timeout envío WebSocket");
    }
