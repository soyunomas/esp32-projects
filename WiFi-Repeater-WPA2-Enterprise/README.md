...
- 🎓 **WPA2-Enterprise** — Soporte EAP-PEAP/TTLS para redes corporativas
- 🔀 **Port Forwarding** — Hasta 5 reglas de redirección de puertos (TCP/UDP)
- 📋 **Log Viewer** — Visor de logs en tiempo real desde la interfaz web
...
| `/api/logs` | GET | Obtener logs del sistema (text/plain) |
...
│   ├── wifi_manager.c/h       # WiFi STA+AP, NAPT, ping, EAP, port forwarding
│   ├── web_server.c/h         # HTTP server + API REST + Auth + OTA
│   ├── config_storage.c/h     # Persistencia NVS (WiFi + EAP + port fwd + web credentials)
│   ├── log_buffer.c/h         # Buffer circular de logs + hook esp_log
