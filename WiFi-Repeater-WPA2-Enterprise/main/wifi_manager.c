...
static void apply_port_forwarding(void)
{
    for (int i = 0; i < CFG_PORT_FWD_MAX; i++) {
        ...
        if (ip_portmap_add(proto, sta_ip, ext_port, dest_ip, int_port)) {
            char ip_str[16];
            esp_ip4_addr_t ip = { .addr = dest_ip };
            snprintf(ip_str, sizeof(ip_str), IPSTR, IP2STR(&ip));
            ESP_LOGI(TAG, "Port forward: :%d -> %s:%d (%s)",
                     ext_port, ip_str, int_port,
                     proto == 17 ? "UDP" : "TCP");
        } else {
            ESP_LOGW(TAG, "Port forward rule %d failed", i);
        }
    }
}
