...
## Sesión 5 - 2026-04-13 (WPA2-Enterprise + Port Forwarding + Log Viewer)

### ✅ Completado
- [x] **WPA2-Enterprise (EAP-PEAP/TTLS)**: campos `sta_eap_enabled/identity/username/password` en config, persistencia NVS, `esp_eap_client` en STA, sin validación CA
- [x] **Port Forwarding**: hasta 5 reglas (proto/ext_port/int_ip/int_port), persistencia NVS como blob, `ip_portmap_add` tras NAPT enable
- [x] **Log Viewer**: buffer circular 4KB en RAM, hook `esp_log_set_vprintf`, endpoint `GET /api/logs` (text/plain), panel web con auto-refresh 3s
- [x] **API Backend**: `GET/POST /api/config` actualizado con campos EAP y port forwarding (flat keys pf0..pf4), `GET /api/logs` nuevo endpoint
- [x] **Web UI**: toggle WPA2-Enterprise con campos EAP, tabla Port Forwarding dinámica (max 5 reglas), pestaña Logs con visor monospace y auto-refresh
- [x] **sdkconfig.defaults**: `CONFIG_ESP_WIFI_ENTERPRISE_SUPPORT=y`
- [x] **CMakeLists.txt**: añadidos `log_buffer.c`, `app_update`, `mbedtls`, `wpa_supplicant`
- [x] Compilación exitosa — firmware 1013KB (43% libre)

## Sesión 6 - 2026-04-13 (Bugfix: Ping + Client IPs)

### ✅ Completado
- [x] **BUG FIX: Ping fallaba con `create socket failed: -1`** — Causa raíz: sockets lwIP agotados (máx 10) y peticiones concurrentes sin serialización. Fix: subido `CONFIG_LWIP_MAX_SOCKETS` de 10 a 16 y añadido mutex (`s_ping_mutex`) para serializar peticiones de ping
- [x] **BUG FIX: Clientes mostraban IP `0.0.0.0`** — Causa raíz: `set_dhcps_dns()` hacía `dhcps_stop/start` al recibir IP STA, lo que destruía la tabla de leases DHCP (los clientes ya conectados perdían su IP asignada). Fix: configurar `DHCPS_OFFER_DNS` una sola vez en `wifi_manager_init` y luego solo actualizar DNS con `esp_netif_set_dns_info` sin reiniciar el DHCP server
- [x] **Binarios actualizados** en `firmware/` — 1013KB (43% libre)
- [x] Verificado en hardware real: ping a 8.8.8.8 = 31ms, clientes muestran IP correcta (192.168.4.x)

### 🔲 TODO (próximas sesiones)
- [ ] Probar EAP en red corporativa real
- [ ] Probar port forwarding con servicio interno
